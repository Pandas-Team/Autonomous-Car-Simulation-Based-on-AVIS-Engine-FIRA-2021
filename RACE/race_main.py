...
try:
    while(True):  
        ...
        if(counter > 4):
            ...
            mask = cv2.inRange(hsv_frame, np.array([110,10,25]), np.array([120,50,60]))
            ...
            if len(sorted_lanes) > 1:
                ...
                right_lane_mask = cv2.morphologyEx(right_lane_mask, cv2.MORPH_CLOSE, kernel)
                left_lane_mask = cv2.morphologyEx(left_lane_mask, cv2.MORPH_CLOSE, kernel)
            ...
            obs_yellow = np.mean(np.where(yellow_mask[50:150,:]>0), axis=1)[1]
            obs_yellow = np.nan_to_num(obs_yellow, nan = 128)

            # obstacle
            obstacle_mask = cv2.inRange(hsv_frame, np.array([90,5,110]), np.array([110,20,155]))
