...
try:
    while(True):  
        ...
        if(counter > 4):
            ...
            try:
                right_lane_mask = cv2.drawContours(np.zeros((70,256)), lane_contours, sorted_areas[-1], 255, -1)
                left_lane_mask = cv2.drawContours(np.zeros((70,256)), lane_contours, sorted_areas[-2], 255, -1)
            except:
                pass
